...
def embed(texts: list[str], task: str = "RETRIEVAL_DOCUMENT") -> list[list[float]]:
    ...
    for i in range(0, len(texts), 32):
        res = retry(lambda: _post("/api/embed", {"model": config.EMBED_MODEL, "input": texts[i:i + 32], "keep_alive": config.OLLAMA_KEEP_ALIVE}))
...
def generate(prompt: str, model: str | None = None) -> str:
    res = retry(lambda: _post("/api/generate", {
        ...
        "prompt": prompt,
        "stream": False,
        "keep_alive": config.OLLAMA_KEEP_ALIVE,  # short by default to free VRAM; a reload costs ~2 s
